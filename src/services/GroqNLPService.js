...
class GroqNLPService {
  constructor() {
    // Groq usa API compatível com OpenAI — só muda baseURL e key
    this.client = new OpenAI({
      apiKey: process.env.GROQ_API_KEY,
      baseURL: 'https://api.groq.com/openai/v1',
    });
    this.model = 'llama-3.3-70b-versatile';
    console.log('✅ GroqNLPService inicializado (LLaMA 3.3 70B)');
  }

  /**
   * Processa mensagem usando LLaMA quando o regex falha.
...
- Se mencionar saldo/quanto tenho: intention=balance, type=query`
      ...
      if (!jsonMatch) {
        console.error('⚠️ Groq NLP: resposta não é JSON:', content);
      ...
      console.error('❌ Groq NLP erro:', error.message);
      return this.fallbackResult(message);
    }
  }
